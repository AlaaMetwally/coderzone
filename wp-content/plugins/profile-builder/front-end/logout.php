<?php
    ...
    function wppb_front_end_logout( $atts ) {
        ...
        extract( shortcode_atts( array( 'text' => sprintf( __('%s. ','profile-builder') ,$current_user->user_login) , 'redirect' => '', 'redirect_url' => wppb_curpageurl(), 'redirect_priority' => 'normal', 'link_text' => __('Log out &raquo;','profile-builder')), $atts ) );
        if( ! empty( $redirect ) ) {
        ...
        $text = apply_filters( 'wppb_front_end_logout_text', str_replace( $meta_tags, $meta_tags_values, $text ), $current_user );
              ...
              return '<li class="menu-item menu-item-type-taxonomy menu-item-object-category" style="height:60px;"><p class="wppb-front-end-logout">' .  $logout_link . '</p></li>';
    }